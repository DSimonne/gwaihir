...
print('run', 'max_reconst', 'obj_std', 'obj_average',
      'LLKf', 'LLK', 'nb_point_support')
for name in files:
    data = h5py.File(name, "r")
    nb_point_support = data['entry_1/image_1/process_1/results/nb_point_support'][...]
    run = int(name.split('/')[-1].split('Run')[1].split('_')[0])
    # Poisson free log-likelihood
    LLKf = data['entry_1/image_1/process_1/results/free_llk_poisson'][...]
    # Poisson log-likelihood
    LLK = data['entry_1/image_1/process_1/results/llk_poisson'][...]
    reconst = data['entry_1/data_1/data'][:]
    ...
    print(run, ' ', np.max(np.abs(reconst)), ' ', obj_std, ' ',
          obj_average, ' ', LLKf, ' ', LLK, ' ', nb_point_support, ' ')
    ...
    axes[1, 1].grid()
    axes[2, 0].scatter(run, np.max(np.abs(reconst)),
                       c='red', marker='+', linewidth=3)
    axes[2, 0].grid()
...
for name in files:
    ...
    if(crop_output):
        ...
        x, y, z = support.sum(axis=1).sum(axis=1)[0:int(cx)], support.sum(
            axis=2).sum(axis=0)[0:int(cy)], support.sum(axis=0).sum(axis=0)[0:int(cz)]
        xmin, ymin, zmin = (x != 0).argmax(
            axis=0) - 5, (y != 0).argmax(axis=0) - 5, (z != 0).argmax(axis=0) - 5
        x, y, z = support.sum(axis=1).sum(axis=1)[int(cx):-1], support.sum(axis=2).sum(
            axis=0)[int(cy):-1], support.sum(axis=0).sum(axis=0)[int(cz):-1]
        xmax, ymax, zmax = (x == 0).argmax(axis=0) + int(cx) + 5, (y == 0).argmax(
            axis=0) + int(cy) + 5, (z == 0).argmax(axis=0) + int(cz) + 5
    ...
    plt.grid()
    plt.subplot(3, 3, 7)
    plt.imshow((phase*support)[int(cx), ymin:ymax,
               zmin:zmax], cmap=segmented_cmap)
    plt.xlabel("z")
    plt.ylabel("y")
    plt.colorbar()
    plt.axis('tight')
    plt.grid()
    plt.subplot(3, 3, 8)
    plt.imshow((phase*support)[xmin:xmax, int(cy),
               zmin:zmax], cmap=segmented_cmap)
    ...
    plt.grid()
    plt.subplot(3, 3, 9)
    plt.imshow((phase*support)[xmin:xmax, ymin:ymax,
               int(cz)], cmap=segmented_cmap)
    plt.xlabel("y")
    plt.ylabel("x")
    plt.colorbar()
    plt.axis('tight')
