...
class Dataset():
    ...
    def to_gwr(self):
        ...
        with h5py.File(f"{self.scan_folder}{self.sample_name}{self.scan}.h5", mode="w") as f:
            ...
            try:
                support.create_dataset(
                    "support_threshold", data=self.support_threshold)
                support.create_dataset(
                    "support_only_shrink", data=self.support_only_shrink)
                support.create_dataset(
                    "support_update_period", data=self.support_update_period)
                support.create_dataset(
                    "support_smooth_width", data=self.support_smooth_width)
                support.create_dataset(
                    "support_post_expand", data=self.support_post_expand)
            except AttributeError:
            ...
            try:
                ...
                data.create_dataset("auto_center_resize",
                                    data=self.auto_center_resize)
            ...
            try:
                algorithms.create_dataset(
                    "use_operators", data=self.use_operators)
                algorithms.create_dataset(
                    "operator_chain", data=self.operator_chain)
            ...
            try:
                ...
                instrument.create_dataset(
                    "pixel_size_detector", data=self.pixel_size_detector)
            except AttributeError:
            ...
            try:
                filtering.create_dataset(
                    "filter_criteria", data=self.filter_criteria)
            ...
            try:
                reciprocal_space.create_dataset("data",
                                                data=np.load(self.iobs)[
                                                    "data"],
                                                chunks=True,
                                                shuffle=True,
                                                compression="gzip")
                reciprocal_space.create_dataset("mask",
                                                data=np.load(self.mask)[
                                                    "mask"],
                                                chunks=True,
                                                shuffle=True,
                                                compression="gzip")
            ...
            try:
                ...
                reciprocal_space["detector_distance"] = h5py.SoftLink(
                    "/parameters/phase_retrieval/instrument/sdd")
                reciprocal_space["pixel_size_detector"] = h5py.SoftLink(
                    "/parameters/phase_retrieval/instrument/pixel_size_detector")
                reciprocal_space["energy"] = h5py.SoftLink(
                    "/parameters/preprocessing/orthogonalisation/linearized_transformation_matrix/energy")
                reciprocal_space.create_dataset(
                    "wavelength", data=self.wavelength)
            except:
