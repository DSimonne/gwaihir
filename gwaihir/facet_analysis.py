...
class Facets(object):
    ...
    def load_vtk(
        ...
        self.vtk_data['x'] = [vtkdata.GetPoint(
            i)[0] for i in range(vtkdata.GetNumberOfPoints())]
        self.vtk_data['y'] = [vtkdata.GetPoint(
            i)[1] for i in range(vtkdata.GetNumberOfPoints())]
        self.vtk_data['z'] = [vtkdata.GetPoint(
            i)[2] for i in range(vtkdata.GetNumberOfPoints())]
        self.vtk_data['strain'] = [pointData.GetArray('strain').GetValue(
            i) for i in range(vtkdata.GetNumberOfPoints())]
        self.vtk_data['disp'] = [pointData.GetArray('disp').GetValue(
            i) for i in range(vtkdata.GetNumberOfPoints())]

        # Get cell data
        cellData = vtkdata.GetCellData()

        self.vtk_data['facet_probabilities'] = [cellData.GetArray(
            'FacetProbabilities').GetValue(i) for i in range(vtkdata.GetNumberOfCells())]
        self.vtk_data['facet_id'] = [cellData.GetArray('FacetIds').GetValue(
            i) for i in range(vtkdata.GetNumberOfCells())]
        self.vtk_data['x0'] = [vtkdata.GetCell(i).GetPointId(
            0) for i in range(vtkdata.GetNumberOfCells())]
        self.vtk_data['y0'] = [vtkdata.GetCell(i).GetPointId(
            1) for i in range(vtkdata.GetNumberOfCells())]
        self.vtk_data['z0'] = [vtkdata.GetCell(i).GetPointId(
            2) for i in range(vtkdata.GetNumberOfCells())]
        ...
        self.field_data['facet_id'] = [fieldData.GetArray(
            'FacetIds').GetValue(i) for i in range(self.nb_facets)]
        ...
        self.field_data['n0'] = [fieldData.GetArray(
            'facetNormals').GetValue(3*i) for i in range(self.nb_facets)]
        self.field_data['n1'] = [fieldData.GetArray(
            'facetNormals').GetValue(3*i+1) for i in range(self.nb_facets)]
        self.field_data['n2'] = [fieldData.GetArray(
            'facetNormals').GetValue(3*i+2) for i in range(self.nb_facets)]
        self.field_data['c0'] = [fieldData.GetArray(
            'FacetCenters').GetValue(3*i) for i in range(self.nb_facets)]
        self.field_data['c1'] = [fieldData.GetArray(
            'FacetCenters').GetValue(3*i+1) for i in range(self.nb_facets)]
        self.field_data['c2'] = [fieldData.GetArray(
            'FacetCenters').GetValue(3*i+2) for i in range(self.nb_facets)]
        self.field_data['interplanar_angles'] = [fieldData.GetArray(
            'interplanarAngles').GetValue(i) for i in range(self.nb_facets)]
        self.field_data['abs_facet_size'] = [fieldData.GetArray(
            'absFacetSize').GetValue(i) for i in range(self.nb_facets)]
        self.field_data['rel_facet_size'] = [fieldData.GetArray(
            'relFacetSize').GetValue(i) for i in range(self.nb_facets)]
        ...
        normals = {f"facet_{row.facet_id}":  np.array(
            [row['n0'], row['n1'], row['n2']]) for j, row in self.field_data.iterrows()}
    ...
    def rotate_particle(self):
        ...
        normals = {f"facet_{row.facet_id}":  np.array(
            [row['n0'], row['n1'], row['n2']]) for j, row in self.field_data.iterrows() if row.facet_id != 0}
    ...
    def fixed_reference(self,
        ...
        self.planar_dist = self.lattice / \
            np.sqrt(
                self.hkl_reference[0]**2+self.hkl_reference[1]**2+self.hkl_reference[2]**2)
        ...
        if plot == True:
            ...
            ax.set_title(
                "Interplanar angles between [111] and other possible facets", fontsize=self.title_fontsize)
    ...
    def extract_facet(self,
        ...
        results = {}
        results['x'], results['y'], results['z'] = np.zeros(len(voxel_indices_new)), np.zeros(
            len(voxel_indices_new)), np.zeros(len(voxel_indices_new))
        results['strain'], results['disp'] = np.zeros(
            len(voxel_indices_new)), np.zeros(len(voxel_indices_new))
    ...
    def plot_strain(self,
        ...
        # 3D strain
        fig_name = 'strain_3D_' + self.hkls + \
            self.comment + '_' + str(self.strain_range)
        ...
        # Average strain
        fig_name = 'strain_3D_avg_' + self.hkls + \
            self.comment + '_' + str(self.strain_range_avg)
        fig = plt.figure(figsize=figsize)
        ax = fig.add_subplot(projection='3d')
    ...
    def plot_displacement(
        ...
        # Average disp
        fig_name = 'disp_3D_avg_' + self.hkls + \
            self.comment + '_' + str(self.disp_range_avg)
        fig = plt.figure(figsize=figsize)
        ax = fig.add_subplot(projection='3d')
    ...
    def evolution_curves(
        ...
        for j, row in self.field_data.iterrows():
            ax.errorbar(row['facet_id'], row['disp_mean'],
                        row['disp_std'], fmt='o', label=row["legend"])

        ax.set_title("Average displacement vs facet index",
                     fontsize=self.title_fontsize)
        ...
        ax.legend(bbox_to_anchor=(1, 1), loc='upper left',
                  borderaxespad=0, fontsize=self.legend_fontsize, ncol=ncol)
        ...
        for j, row in self.field_data.iterrows():
            ax.errorbar(row['facet_id'], row['strain_mean'],
                        row['strain_std'], fmt='o', label=row["legend"])
        ...
        for j, row in self.field_data.iterrows():
            try:
                lx, ly, lz = float(row.legend.split()[0]), float(
                    row.legend.split()[1]), float(row.legend.split()[2])
                if (lx >= 0 and ly >= 0):
                    fmt = 'o'
                if (lx >= 0 and ly <= 0):
                    fmt = 'd'
                ...
                fmt = '+'
            ax0.errorbar(row['interplanar_angles'], row['disp_mean'],
                         row['disp_std'], fmt=fmt, capsize=2, label=row["legend"])
        ax0.set_ylabel('Retrieved <disp> (A)', fontsize=self.axes_fontsize)
        ax0.legend(loc='upper left', bbox_to_anchor=(1, 1), ncol=1,
                   fancybox=True, shadow=True, fontsize=self.legend_fontsize+3)
        ...
        for j, row in self.field_data.iterrows():
            try:
                ...
                fmt = '+'
            ax1.errorbar(row['interplanar_angles'], row['strain_mean'],
                         row['strain_std'], fmt=fmt, capsize=2, label=row["legend"])
        ...
        for j, row in self.field_data.iterrows():
            try:
                ...
                fmt = '+'
            ax2.plot(row['interplanar_angles'],
                     row['rel_facet_size'], 'o', label=row["legend"])
